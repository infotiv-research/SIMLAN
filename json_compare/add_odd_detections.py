...
def load_odd_detection_files(folder_path: str) -> Dict[str, Dict]:
    ...
    for json_file in json_files:
        ...
        filename = os.path.splitext(os.path.basename(json_file))[0]  # Get filename without extension
        
        if file_data:
            print(f"  Loaded {len(file_data)} entries from {os.path.basename(json_file)}")
...
def add_odd_detections(combined_data: List[dict], odd_data: Dict[str, Dict]) -> List[dict]:
...
def process_combined_file(combined_file_path: str, odd_folder_path: str, output_file: str = None):
    ...
    if not odd_data:
        print("Warning: No odd detection data loaded. Continuing with empty odd_detections lists.")
    ...
    try:
        ...
        # Show statistics
        timestamps_with_odd = sum(1 for entry in enhanced_data if len(entry.get("odd_detections", [])) > 0)
        print(f"Timestamps with odd detections: {timestamps_with_odd}/{len(enhanced_data)}")
        
        # Count total files with pred==true across all timestamps
        total_files_with_pred = sum(len(entry.get("odd_detections", [])) for entry in enhanced_data)
...
def main():
    parser = argparse.ArgumentParser(description="Add odd detections to combined JSON file")
    parser.add_argument("combined_file", help="Path to combined JSON file")
    parser.add_argument("odd_folder", nargs="?", default="odd_scores", help="Path to folder containing odd detection JSON files (default: odd_scores)")
    parser.add_argument("-o", "--output", help="Output file path (default: input_with_odd.json)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("  python add_odd_detections.py combined_file.json odd_detections_folder/")
        print("  python add_odd_detections.py combined.json odd_folder/ -o enhanced_output.json")
        print()
        print("Input structure:")
        print("  Combined file: List of entries with 'time_stamp' and 'object_list'")
        print("  Odd folder: Contains JSON files with timestamp keys and 'pred'/'ood_score' values (default: odd_scores/)")
        print()
