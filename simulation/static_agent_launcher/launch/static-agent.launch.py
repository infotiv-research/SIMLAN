...
def launch_setup(context, *args, **kwargs):
    ...
    camera_update_rate = LaunchConfiguration("camera_update_rate").perform(context)
    camera_enabled_ids = camera_enabled_ids.split(" ")
    camera_streams_array = camera_streams.split(" ")
    ...
    # <xacro:property name="cameras" default="image depth semantic" />
    actions = []
    # Configure the node
    ...
    for camera_id in camera_enabled_ids:
        for streams in camera_streams_array:
            if streams == "image":
    ...
    ros_gz_images = Node(
        ...
        output="screen",
        arguments=[*camera_parameters, "--ros-args", "--log-level", log_level],
