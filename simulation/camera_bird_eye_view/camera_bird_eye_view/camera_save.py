...
class CameraSaver(Node):
    def __init__(self):
        ...
        # Read the parameter
        camera_ids_param = self.get_parameter('camera_ids').get_parameter_value().string_value
        camera_ids = [int(x.strip()) for x in camera_ids_param.replace(',', ' ').split() if x.strip()]
        ...
        # Output folder
        self.output_dir = os.path.expanduser("/home/ros/src/simulation/camera_bird_eye_view/camera_images")
        ...
        for cam in self.cameras:
            sub = self.create_subscription(
                Image,
                cam["topic"],
                lambda msg, c=cam: self.image_callback(msg, c),
                10
            )
        ...
        if now - self.last_saved >= self.save_interval:
            for cam_name, (msg, cam) in self.latest_images.items():
                try:
                    ...
                    filename = os.path.join(cam["save_dir"], f"{cam_name}_{timestamp}.png")
                    ...
                    if cam["type"] == "depth":
                        ...
                        cv_image_norm = cv2.normalize(cv_image_clipped, None, 0, 255, cv2.NORM_MINMAX)
                        depth_gray = cv_image_norm.astype('uint8')
                        cv2.imwrite(filename, depth_gray)
                        self.get_logger().info(f"[{cam_name}] Saved grayscale depth: {filename}")

                    elif cam["type"] == "segmentation":
                        cv2.imwrite(filename, cv_image)
                        self.get_logger().info(f"[{cam_name}] Saved segmentation: {filename}")
