...
def generate_launch_description():
    ...
    filepath_config_twist_mux = PathJoinSubstitution(
        [FindPackageShare("pallet_truck_control"), "config", "twist_mux.yaml"]
    )

    filepath_config_interactive_markers = PathJoinSubstitution(
        [
            FindPackageShare("pallet_truck_control"),
            "config",
    ...
    node_twist_mux = Node(
        ...
        output="screen",
        remappings={
            ("/cmd_vel_out", "/pallet_truck_velocity_controller/cmd_vel_unstamped")
        },
