    ...
    def camera_aruco_pose_callback(self):
        ...
        for camera_id in self.camera_enabled_ids:
            for marker_id in markers_of_interest:
                ...
                if self.tf_buffer.can_transform(f"robot_agent_{marker_id}/odom", target_frame, rclpy.time.Time()):
                    t = self.tf_buffer.lookup_transform(f"robot_agent_{marker_id}/odom", target_frame, rclpy.time.Time())
