...
class TeleportActionServer(Node):
    ...
    def __init__(self):
        ...
        self._action_server = ActionServer(
            self, TeleportRobot, "teleport_robot", self.execute_callback
        )
    ...
    async def execute_callback(self, goal_handle):
        self.get_logger().info(
            f"Received teleport request for {goal_handle.request.robot_name}"
        )
        ...
        if future.result() is not None:
            ...
            goal_handle.succeed()
            return TeleportRobot.Result(
                success=True, message="Teleportation successful"
            )
        else:
