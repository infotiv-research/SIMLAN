...
class TTCMinkowskiBroadcaster(Node):
    def __init__(self):
        ...
        self.declare_parameter("pallet_truck_size", [1.1, 0.9])
        ...
        self.get_logger().info(
            "Initialized TTC node with robots: "
            f"Robot1 '{self.robot1_name}' size: {s1[0]:.2f} x {s1[1]:.2f} m | "
            f"Robot2 '{self.robot2_name}' size: {s2[0]:.2f} x {s2[1]:.2f} m"
        )
