...
def combine_json_files(
    file1_data: List[dict], file2_data: List[dict], merge_mode: str = "timestamp"
) -> List[dict]:
    """
    ...
    if merge_mode == "concatenate":
        ...
        for entry in file1_data:
            ...
            if timestamp is not None and timestamp in combined_dict:
                ...
                combined_dict[timestamp] = {"time_stamp": timestamp, "object_list": []}
...
def combine_files(
    ...
    try:
        ...
        if merge_mode == "timestamp":
            timestamps_file1 = set(
                entry.get("time_stamp")
                for entry in file1_data
                if entry.get("time_stamp")
            )
            timestamps_file2 = set(
                entry.get("time_stamp")
                for entry in file2_data
                if entry.get("time_stamp")
            )
            ...
            print(
                f"Unique timestamps from file 1: {len(timestamps_file1 - timestamps_file2)}"
            )
            print(
                f"Unique timestamps from file 2: {len(timestamps_file2 - timestamps_file1)}"
            )
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", help="Output file path (default: file1_combined.json)"
    )
    parser.add_argument(
        "-m",
        "--mode",
        choices=["timestamp", "concatenate"],
        default="timestamp",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("Merge modes:")
        print(
            "  timestamp (default): Merge predictions into objects by track_id at same timestamp"
        )
