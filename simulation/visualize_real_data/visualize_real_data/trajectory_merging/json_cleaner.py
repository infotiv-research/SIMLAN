...
def remove_field_from_objects(
    data: Any, field_to_remove: str, container_key: str = None
) -> int:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Remove named fields from objects in a JSON file"
    )
    ...
    parser.add_argument(
        "-c",
        "--container",
        help="Container key that holds the objects (e.g., 'object_list', 'predictions')",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print(
            "  python json_cleaner.py data.json field_name -c predictions -o cleaned_data.json"
        )
