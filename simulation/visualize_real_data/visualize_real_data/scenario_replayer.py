...
class ScenarioReplayTeleport(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "robot_namespaces",
            ["robot_agent_1", "robot_agent_2", "robot_agent_3", "robot_agent_4"],
        )
        ...
        self.extracted_fps = (
            self.get_parameter("extracted_fps").get_parameter_value().double_value
        )
        self.use_cmd_vel = (
        ...
        if self.use_cmd_vel:
            ...
            self.get_logger().info(
                f"Max velocities: linear={self.max_linear_velocity:.1f} m/s, angular={self.max_angular_velocity:.1f} rad/s"
            )
        ...
        self.teleport_client = self.create_client(
            SetEntityPose, "/world/default/set_pose", callback_group=self.callback_group
        )
    ...
    def apply_orientation_policy(self, pose: Pose) -> Pose:
        """Apply ignore_orientation policy to a pose"""
        if self.ignore_orientation:
            # Keep position but reset orientation to identity (no rotation)
            modified_pose = Pose()
            modified_pose.position = pose.position
            modified_pose.orientation.w = 1.0  # Identity quaternion
            modified_pose.orientation.x = 0.0
            modified_pose.orientation.y = 0.0
    ...
    def transform_pose_to_target_frame(self, pose: Pose, source_frame: str) -> Pose:
        ...
        try:
            ...
            self.get_logger().error(
                f"Failed to transform pose from {source_frame} to {self.target_frame_id}: {e}"
            )
            ...
            self.get_logger().error(
                f'Error details: transformed_pose_stamped type = {type(transformed_pose_stamped) if "transformed_pose_stamped" in locals() else "not defined"}'
            )
            return pose
    ...
    def get_parking_pose(self, index: int) -> Pose:
        ...
        return self.apply_orientation_policy(pose)
    ...
    def entity_callback(self, msg: MarkerArray):
        ...
        for marker in msg.markers:
            ...
            world_pose = self.transform_pose_to_target_frame(
                marker.pose, marker.header.frame_id
            )

            # Apply orientation policy
            world_pose = self.apply_orientation_policy(world_pose)
            ...
            if marker_id in self.id_to_robot_mapping:
                ...
                if self.use_cmd_vel:
                    if robot_name in self.robot_current_positions:
                        self.robot_previous_positions[robot_name] = (
                            self.robot_current_positions[robot_name]
                        )
                ...
                if marker_id not in self.active_marker_ids:
                    ...
                    self.get_logger().info(
                        f"Reactivated robot {robot_name} for marker ID {marker_id}"
                    )
            else:
                ...
                if available_robot:
                    ...
                    self.get_logger().info(
                        f"Permanently assigned robot {available_robot} to marker ID {marker_id}"
                    )
                else:
                    self.get_logger().warn(
                        f"No available robots to assign to marker ID {marker_id}"
                    )
        ...
        for marker_id in inactive_marker_ids:
            if marker_id in self.id_to_robot_mapping:
                ...
                self.get_logger().info(
                    f"Parked robot {robot_name} - marker ID {marker_id} no longer active"
                )
    ...
    def teleport_robot_direct(self, robot_name: str, pose: Pose):
        ...
        self.get_logger().debug(
            f"Teleporting {robot_name} to {self.target_frame_id} pose: x={pose.position.x:.3f}, y={pose.position.y:.3f}, z={pose.position.z:.3f}"
        )

        # Call service asynchronously
        future = self.teleport_client.call_async(request)
        future.add_done_callback(lambda f: self.teleport_callback(f, robot_name))
    ...
    def teleport_callback(self, future, robot_name: str):
        ...
        try:
            ...
            self.get_logger().error(
                f"Teleport service call failed for {robot_name}: {e}"
            )

    def get_robot_status(self):
        """Debug method to print current robot status"""
        active_robots = [
            self.id_to_robot_mapping[marker_id] for marker_id in self.active_marker_ids
        ]
    ...
    def calculate_cmd_vel(self, robot_name: str, target_pose: Pose) -> Twist:
        ...
        if robot_name not in self.robot_previous_positions:
            ...
            self.get_logger().debug(
                f"No previous position for {robot_name}, sending zero velocity"
            )
            return twist
        ...
        dy_local = -dx_global * math.sin(current_yaw) + dy_global * math.cos(
            current_yaw
        )
    ...
    def send_cmd_vel(self, robot_name: str, twist: Twist):
        ...
        if robot_name in self.cmd_vel_publishers:
            ...
            self.get_logger().debug(
                f"Sent cmd_vel to {robot_name}: linear={twist.linear.x:.3f}, angular={twist.angular.z:.3f}"
            )
        else:
