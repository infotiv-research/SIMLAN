...
class PrepareRealData(Node):
    def __init__(self):
        ...
        self.image_scale = (
            self.get_parameter("image_scale").get_parameter_value().double_value
        )
        self.images_folder_path = (
            self.get_parameter("images_folder").get_parameter_value().string_value
        )
        entity_topic = (
        ...
        self.set_frames = (
            self.get_parameter("set_frames").get_parameter_value().bool_value
        )
        self.frames_to_process = (
            self.get_parameter("frames_to_process").get_parameter_value().integer_value
        )
        self.processing_time = (
            self.get_parameter("processing_time_limit")
            .get_parameter_value()
            .double_value
        )
        self.frame_id = (
    ...
    def execute_callbacks(self):
        ...
        if self.start_timestamp_ms or self.end_timestamp_ms:
            ...
            for frame in data_dict:
                ...
                if odd_detections:
                    ...
                    break
            if not found_odd:
                self.get_logger().warn("No ODD detections found in the selected timestamp range!")
            else:
                self.get_logger().info("ODD detections present in the selected timestamp range.")
        ...
        if self.set_frames:
            ...
            if self.frames_to_process <= num_frames:
                ...
                self.get_logger().warn(
                    f"frames_to_process assigned value larger than available data,"
                )

            self.get_logger().warn(
                f"{num_frames}/{len(data_dict)} available frames will be processed"
            )
        ...
        for frame in range(num_frames):
            ...
            image_path = Path(
                self.images_folder_path, str(data_dict[frame]["time_stamp"]) + ".jpg"
            )  # sorted_images[frame]
            ...
            for object_info in frame_info["object_list"]:
                ...
                if isinstance(object_info, dict):
                    ...
                    yaw = self.track_id_yaw_history.get(object_id, 0.0)  # or could be from route/orientation data
                ...
                if object_id is not None:
                    ...
                    entity_msg, text_msg = self._create_marker_message(
                        object_id, object_pos, display_label=object_info.get("display_label", "")
                    )
        ...
        if self.preprocess_all_data:
            self.get_logger().info(
                "Preprocessing complete, publishing all preprocessed data using original timing..."
            )
            ...
            for frame in range(num_frames - 1):
                ...
                self.odd_publisher_.publish(String(data=json.dumps(preprocessed_odd[frame])))
                # Use the actual time interval from the original data
                frame_interval = self.dt_intervals[
                    frame
                ]  # dt_intervals[0] is the interval between frame 0 and 1
                self.get_logger().info(
                    f"published {frame+1}/{num_frames} messages (interval: {frame_interval:.3f}s)"
                )
                dt = time.time() - start
            ...
            self.get_logger().info(
                f"published {num_frames}/{num_frames} messages (interval: {frame_interval:.3f}s)"
            )
    ...
    def _wait_for_subscriber(self):
        while (
            self.pc_publisher_.get_subscription_count()
            + self.ent_publisher_.get_subscription_count()
            < 2
        ):
    ...
    def _check_timestamps(self, data_dict):
        ...
        for frame_data in data_dict:
            ...
            try:
                ...
                self.get_logger().error(
                    f"{e_msg}, at frame {frame_data['time_stamp']}: Timestamp not convertible to int"
                )
                continue
        ...
        dt_array = [
            saved_stamps[idx + 1] - saved_stamps[idx]
            for idx in range(len(saved_stamps) - 1)
        ]
        ...
        if len(dt_array) > 0:
            ...
            self.get_logger().info(
                f"  Individual intervals (s): {[f'{dt:.3f}' for dt in self.dt_intervals[:5]]}{'...' if len(self.dt_intervals) > 5 else ''}"
            )
            ...
            self.get_logger().warn(
                "No valid timestamp intervals found, defaulting to 1 fps"
            )
        ...
        if doc["shared"]["extracted_fps"] != self.dt_fps:
            ...
            self.get_logger().info(
                f"Config already contains correct average fps: {self.dt_fps:.2f}"
            )
    ...
    def _create_marker_message(self, object_id, object_pos, display_label=""):
        ...
        entity_msg.text = "id:" + str(
            object_id
        )  # Used to teleport the robots in gazebo
