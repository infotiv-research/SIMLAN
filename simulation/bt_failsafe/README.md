...
In this approach for a new Behavior Tree, the **condition node** named [`StopRobotCondition`](simulation/bt_failsafe/include/bt_failsafe/stop_robot.hpp) is defined and can be triggered when a safety issue occurs. We then added a [Behavior Tree plugin](simulation/bt_failsafe/bt_failsafe_plugins.xml) that calls an action called [CancelControl](https://docs.nav2.org/configuration/packages/bt-plugins/actions/CancelControl.html) when the given condition is triggered.
...
The second plugin we tried is the [`IsStuckCondition`](https://github.com/ros-navigation/navigation2/blob/main/nav2_behavior_tree/plugins/condition/is_stuck_condition.cpp). This plugin checks if the robot is stuck by calculating the deceleration of the robot. If the deceleration is too big, it returns `FAILURE`. This wasn't anything we could use because the acceleration is set to zero as soon as the robot gets out of bounds. So this is probably not suitable for this case.
